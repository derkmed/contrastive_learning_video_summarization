...
class VSum_DataLoader(Dataset):
    ...
    def _words_to_token(self, words):
        ...
        if words:
            we = self._zero_pad_tensor_token(
                torch.LongTensor(words), self.max_words)
            return we
        else:
    ...
    def _get_text(self, caption):
        ...
        if self.num_candidates == 1:
            ...
            words = torch.zeros(self.num_candidates,
                                self.max_words, dtype=torch.long)
            cap_start = self._find_nearest_candidates(cap, ind)
            for i in range(self.num_candidates):
                words[i] = self.words_to_ids(
                    cap["text"].values[max(
                        0, min(len(cap["text"]) - 1, cap_start + i))]
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset == "wikihow":
            ...
            label_scores = (
                torch.FloatTensor(
                    self.annts[video_id]["machine_summary_scores"]) / 0.1
            )
            num_frames_in_video = (
