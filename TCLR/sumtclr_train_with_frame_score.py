...
def train_epoch(scaler, run_id, learning_rate2, epoch, criterion, data_loader, 
                model, optimizer, writer, use_cuda, criterion2):
...
def train_classifier(traintestlist: str, run_id: str, reload: bool, prev_model_filepath: str = '', 
    ...
    if reload:
        ...
        try:
            ...
            model = load_r3d_mlp(saved_model_file=saved_model_file)
            # TCLR originally has this. But, in our experience, the pretrained model
            # still has the architecture weights.
            # model = build_r3d_mlp()
            epoch0 = 0 
            scheduler_epoch = 0
            best_score = 10000
    ...
    for epoch in range(epoch0, n_epochs):
        ...
        start=time.time()
        try:
            model, loss, scaler = train_epoch(scaler, run_id, learning_rate2, epoch, criterion, train_dataloader, 
                                                model, optimizer, writer, use_cuda, criterion2=global_local_temporal_contrastive)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--freeze_tclr", dest='freeze_tclr', type=bool, required=False, default=True)
